...
export const name = 'dsh-preset-subagent'
...
export function apply(ctx, config = {}) {
  const providerName = config.providerName ?? 'preset-mount'
  const toolName = config.toolName ?? 'subagent_as_preset'
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => [{ type: 'text', text: `[subagent_as_preset ${value.preset}${value.model ? ' @ ' + value.provider + '/' + value.model : ''} | ${value.stopReason}]\n${(value.output ?? []).filter((b) => b && b.type === 'text').map((b) => b.text).join('')}` }],
    },
    ...
    async execute(args, exec) {
      ...
      // validate the target preset up front (helpful error lists valid ids)
      try {
        await agentPresets.resolve(args.preset)
      } catch {
        const ids = (await agentPresets.list()).map((p) => p.id).sort()
        throw new Error(`unknown agent preset "${args.preset}"; available presets: ${ids.join(', ') || '(none)'}`)
      }
      ...
      // of letting the child's LLM route fail opaquely.
      if (agentOptions.model !== undefined) {
        ...
        if (llm !== undefined && provider !== undefined && typeof llm.resolveModelInfo === 'function') {
          try {
            ...
            throw new Error(`model "${agentOptions.model}" is not available under provider "${provider}"; available models under ${provider}: ${available}. Pass provider=... to use another provider.`)
          }
        }
      }
      ...
      const run = await ctx.subagents.start(providerName, {
        ...
        maxDepth: typeof args.max_depth === 'number' && Number.isFinite(args.max_depth) ? args.max_depth : defaultMaxDepth,
        signal: exec.signal,
      })
