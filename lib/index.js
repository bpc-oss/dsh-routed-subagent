...
/**
 * Validate the tool_filter parameter shape: { allow?: string[], deny?: string[] }.
 * Both lists must be arrays of non-empty strings; at least one must be present.
 */
function validateToolFilter(filter, toolName) {
  if (typeof filter !== 'object' || filter === null || Array.isArray(filter)) {
    throw new Error(`${toolName}: tool_filter must be an object { allow?: string[], deny?: string[] }`)
  }
  const check = (list, key) => {
    if (list === undefined) return undefined
    if (!Array.isArray(list) || list.some((n) => typeof n !== 'string' || n.length === 0)) {
      throw new Error(`${toolName}: tool_filter.${key} must be an array of non-empty tool names`)
    }
    return [...new Set(list)]
  }
  const allow = check(filter.allow, 'allow')
  const deny = check(filter.deny, 'deny')
  if (allow === undefined && deny === undefined) {
    throw new Error(`${toolName}: tool_filter needs allow and/or deny`)
  }
...
export function apply(ctx, config = {}) {
  ...
  const provider = {
    ...
    async start(request) {
      ...
      const setup = async (childCtx) => {
        ...
        // Ported stock-subagent capabilities: persona override and tool filter
        // apply ON TOP of the mounted preset's composition.
        if (request.persona !== undefined && request.persona !== '') {
          // Same section name/order as the official applyChildComposition.
          childCtx.systemPrompt.section({ name: 'deployment:persona', order: 0, text: request.persona })
        }
        if (request.toolFilter !== undefined && (request.toolFilter.allow !== void 0 || request.toolFilter.deny !== void 0)) {
          childCtx.tools.restrict({
            ...(request.toolFilter.allow !== void 0 ? { allow: request.toolFilter.allow } : {}),
            ...(request.toolFilter.deny !== void 0 ? { deny: request.toolFilter.deny } : {}),
          })
        }
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    name: toolName,
    description: 'Run a subagent fully mounted on a SPECIFIED agent preset, from any session — the child adopts that preset\'s complete composition (persona, prompt sections, skill catalog, tools), unlike the stock subagent tools which inherit this session\'s own preset. BACKGROUND by default: the call RETURNS A JOB ID IMMEDIATELY (like the stock subagent tool) — the child runs independently, aborting this conversation does NOT cancel it, and you can dispatch several in parallel. UNLESS the task explicitly requires the result before you can continue, DO NOT wait: return the job id, continue your other work, and collect results later with job_output (live progress) or stop with job_kill. Set run_in_background: false ONLY when the very next step depends on the child\'s output (the conversation blocks until it returns).',
    parameters: {
      ...
      persona: { type: 'string', description: 'Optional persona text that OVERRIDES the mounted preset\'s persona section (same semantics as the stock subagent persona parameter).' },
      tool_filter: { type: 'object', additionalProperties: false, properties: { allow: { type: 'array', items: { type: 'string' } }, deny: { type: 'array', items: { type: 'string' } } }, description: 'Optional tool mask applied ON TOP of the mounted preset\'s tool surface: { allow?: string[], deny?: string[] } naming global tool names (same semantics as the stock subagent toolFilter).' },
    ...
    async execute(args, exec) {
      ...
      const request = {
        ...
        ...(args.persona !== undefined && args.persona !== '' ? { persona: args.persona } : {}),
      ...
      if (background) {
        ...
        const jobId = jobs.start({
          ...
          run: () => {
            ...
            let child = null // live child agent, captured once the run starts
            const started = Date.now()
            const startPromise = ctx.subagents.start(providerName, { ...request, signal: controller.signal })
            ...
            return {
              ...
              readOutput: () => liveProgress(child, started),
