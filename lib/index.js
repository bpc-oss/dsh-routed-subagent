...
export function apply(ctx, config = {}) {
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      fork: { type: 'boolean', description: 'Default false: fresh child with no inherited conversation. Set true to FORK this conversation — the child is seeded with all COMPLETED turns of this session (the current tool-call turn excluded) so it inherits the conversation context, then mounts the requested preset on top (stronger than the stock subagent_fork, which cannot change the preset).' },
      continuable: { type: 'boolean', description: 'Default false: one-shot child (returns a job id / run result). Set true to return a DURABLE subagent id instead — the child keeps running independently and you continue it later with send_message(subagentId, ...) (multi-turn). The child mounts the requested preset and keeps it across resumes (requires the patched @deepseek-ai/dsh-subagent; the startup assertion warns if the patch is missing). Mutually exclusive with run_in_background: false.' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render: (_args, value) => value.kind === 'background'
        ...
        : value.kind === 'continuable'
          ? [{ type: 'text', text: `started continuable subagent ${value.subagentId} on preset "${value.preset}"${value.model ? ' @ ' + value.provider + '/' + value.model : ''} — continue it with send_message(${value.subagentId}, ...)` }]
    ...
    async execute(args, exec) {
      ...
      const dispatchProvider = args.fork ? forkProviderName : providerName
      const request = {
      ...
      if (args.continuable) {
        // CONTINUABLE: return a durable subagent id immediately — the parent
        // later sends follow-up messages with send_message(subagentId, ...).
        // Requires the patched @deepseek-ai/dsh-subagent so the child mounts
        // the target preset on creation AND on cold resume (descriptor.preset).
