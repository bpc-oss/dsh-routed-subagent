...
// Default turn timeout (30 min). Can be overridden per-request via request.timeoutMs.
...
class CodexAppServerWire {
  ...
  _spawn(bin, cwd, env) {
    ...
    this.child.on('exit', (code, sig) => {
      ...
      // Spontaneous crash: wake every notification waiter too, so run.result
      // settles (interrupted) instead of hanging until the timeout.
...
export class CodexEngineProvider {
  ...
  async start(request) {
    ...
    // cwd resolution order: explicit request.cwd (tool arg) > config.cwd >
    // parent session cwd. The parent-session fallback is LAST because the
    // parent is a DSH conversation — its cwd is the harness runtime dir, NOT
    // the repository under review; a codex run must target the repo.
    const explicitCwd = request.cwd
    const parentCwd = request.parent?.session?.header?.cwd
    if (explicitCwd === undefined && this.config.cwd === undefined && parentCwd === undefined) {
      throw new Error('codex-provider: no working directory — pass cwd=... or delegate from a parent session that has one')
    }
    const cwd = explicitCwd ?? this.config.cwd ?? parentCwd
    ...
    // Re-check after wire init: an abort that fired during startup must not
    // start the turn at all.
    if (request.signal?.aborted) {
      aborted = true
      await disposeWire()
      throw new Error('codex-provider: aborted during startup')
    }
    ...
    const resultPromise = (async () => {
      try {
        ...
        return await this._awaitTurnInternal(wire, tid, tun, () => aborted, progress, request.timeoutMs)
      } finally {
        await disposeWire()
      }
    })()
  ...
  _awaitTurnInternal(wire, threadId, turnId, abortedFn, progress, timeoutMs) {
    return new Promise((resolve, reject) => {
      let final = ''
      let timer = null
      let settled = false
      const limit = Number.isFinite(timeoutMs) && timeoutMs > 0 ? timeoutMs : DEFAULT_TURN_TIMEOUT_MS
      ...
      timer = setTimeout(() => settle(reject, new Error(`codex-provider: timed out waiting for turn/completed (${Math.round(limit / 1000)}s)`)), limit)
    })
  }
  ...
  async prepareContinuable(request) {
    ...
    try {
      ...
      const resume = (content, signal) => this._resume(threadId, cwd, content, signal, timeoutMs)
  ...
  async _resume(threadId, cwd, content, signal, timeoutMs) {
    ...
    let turnId = null
    // Re-check after wire init: a signal that fired during startup must not
    // let the turn proceed and report 'completed'.
    if (signal?.aborted) { await disposeWire(); return { output: [], stopReason: 'aborted' } }
    if (signal) {
    ...
    try {
      ...
      const done = await this._awaitTurnInternal(wire, threadId, turnId, () => aborted || (signal?.aborted ?? false), progress, timeoutMs)
