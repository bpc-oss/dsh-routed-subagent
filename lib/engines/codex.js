...
export class CodexEngineProvider {
  ...
  /** Provider-level teardown (kept for the ctx.effect hook; no shared wire anymore). */
  async dispose() {
    // Nothing shared to kill: every run owns and disposes its own wire.
  }
  ...
  async _resume(threadId, cwd, content, signal) {
    ...
    try {
      ...
      const turnId = turnRes?.turn?.id
      const done = await this._awaitTurnInternal(wire, threadId, turnId, () => signal?.aborted ?? false, progress)
