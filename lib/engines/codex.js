...
export class CodexEngineProvider {
  ...
  /** Kill the shared app-server process. Idempotent; called on plugin teardown. */
  async dispose() {
    if (this._wire) {
      const w = this._wire
      this._wire = null
  ...
  _awaitTurnInternal(wire, threadId, turnId, abortedFn) {
    return new Promise((resolve, reject) => {
      ...
      const off = wire.onNotify((msg) => {
        const p = msg.params ?? {}
        const m = msg.method
        // STRICT turn/thread isolation: codex notifications carry the turn id
        // under `params.turn.id` (and thread id under `params.threadId` /
        // `params.thread.id`), NOT a top-level `params.turnId`. Filtering on
        // the wrong path lets OTHER runs' events leak into this run's output
        // and stopReason (mixed output / bogus interrupted).
